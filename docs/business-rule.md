# 기능별 비즈니스 로직

## Authentication  
 - OIDC(OpenID Connect) 기반의 소셜 로그인을 지원한다.
 - OpenID 공급자가 발급한 ID Token을 검증한 후, 서비스에서 사용할 사용자 계정 생성 및 사용자 인증을 위한 JSON Web Token(Access Token, Refresh Token)을 발급한다.
 - 사용자는 계정 생성에 사용한 OpenID 공급자를 통해서만 로그인할 수 있다.
 - 인증이 필요한 요청은 Access Token을 통해 사용자를 검증한다.
 - Access Token이 만료된 경우 Refresh Token으로 사용자 검증 후 Access Token, Refresh Token을 재발급한다.

## User
 - 사용자는 서비스를 이용하기 위해 로그인해야 한다.
 - 사용자는 최초 로그인 후 프로필을 설정해야 하며, 닉네임은 공백으로 설정할 수 없다.
 - 사용자는 계정아이디 생성 여부를 선택할 수 있다.
 - 사용자는 자신의 정보(프로필정보, 계정아이디)를 수정할 수 있다.
 - 회원탈퇴 시 사용자를 비활성화한다.

## Friend
 - 친구 추가 전 사용자를 검색할 수 있으며, 이미 친구 관계가 생성된 사용자는 검색 결과에서 제외한다.
 - 친구 추가 시 사용자 간 단방향 친구관계를 생성한다.

## Chat 
 - 1:1 채팅의 경우 사용자 간 고유한 키를 생성하여 중복된 채팅방이 생성되지 않도록 한다.
 - 사전에 생성된 채팅방 아이디를 기준으로 메시지를 전송 한다.  
 - 사용자가 채팅방에서 퇴장한 경우, 해당 사용자를 채팅방 멤버에서 비활성화한다. 
 - 퇴장한 사용자가 재입장하는 경우 또는 상대방이 메시지를 보낸 경우, 퇴장한 사용자를 재활성화한다.
 - 채팅방의 활성 멤버가 1명 이상인 경우 방을 유지하며, 모든 멤버가 비활성 상태인 경우 채팅방과 관련된 모든 데이터를 삭제한다.
 - 사용자는 자신이 채팅방의 멤버로 활성화된 시간 이후의 메시지를 확인할 수 있다.
